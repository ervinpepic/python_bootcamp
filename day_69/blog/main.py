...
def register():
    ...
    if form.validate_on_submit():
        if User.query.filter_by(email=form.email.data).first():
            flash(message="You are already registered with that email. Login instead.")
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user_to_login and check_password_hash(
                pwhash=user_to_login.password,
...
def show_post(post_id):
    ...
    if form.validate_on_submit():
        if current_user.is_authenticated:
            ...
            flash(
                message="You need to be logged in to leave a comment. Please login first")
            return redirect(location=url_for("login"))
